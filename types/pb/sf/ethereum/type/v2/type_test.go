...
func TestCombinePolygonSystemTransactions(t *testing.T) {
	...
	normalTrx := func(id string) *TransactionTrace {
		return &TransactionTrace{
			Hash: B(id),
		}
	}
	systemTrx := func(id string, beginOrdinal, endOrdinal uint64, calls []*Call) *TransactionTrace {
		return &TransactionTrace{
	...
	call := func(index, parent, depth uint32, beginOrdinal, endOrdinal uint64) *Call {
		call := &Call{
			Index:        index,
			ParentIndex:  parent,
			Depth:        depth,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			var systemTrx *TransactionTrace
			var trxIDs []string
			for _, trx := range out {
