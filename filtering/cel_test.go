...
func BenchmarkCELFilter(b *testing.B) {
	tests := []struct {
		name          string
		code          string
		trace         *pbcodec.Transaction
		expectedMatch bool
		withCache     bool
	}{
		{
			"CELFilter FROM 100 without cache",
			`from in ` + listOf100AddressesCEL,
			&pbcodec.Transaction{
				From:  toBytes(nil, "aaaaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbb"),
				Input: toBytes(nil, "12345678"),
			},
			true,
			false,
		},
		{
			"CELFilter FROM 100 with cache",
			`from in ` + listOf100AddressesCEL,
			&pbcodec.Transaction{
				From:  toBytes(nil, "aaaaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbb"),
				Input: toBytes(nil, "12345678"),
		...
		{
			"CELFilter FROM 1 without cache",
			`from == "0xaaaaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbb"`,
			&pbcodec.Transaction{
				From:  toBytes(nil, "aaaaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbb"),
				Input: toBytes(nil, "12345678"),
		...
		{
			"CELFilter ERC20_FROM 100 without cache",
			`erc20_from in ` + listOf100AddressesCEL,
			&pbcodec.Transaction{
				From: toBytes(nil, "aaaaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbb"),
				Input: toBytes(nil, transferMethod+
					leftPad32b("1111111111111111111111111111111111111111")+
					leftPad32b("00")),
			},
			true,
		...
		{
			"CELFilter ERC20_FROM 100 with cache",
			`erc20_from in ` + listOf100AddressesCEL,
			&pbcodec.Transaction{
				From: toBytes(nil, "aaaaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbb"),
				Input: toBytes(nil, transferMethod+
	...
	for _, test := range tests {
		...
		b.Run(test.name, func(b *testing.B) {
			// use my trxFilter with Match with a cache or no cache
			//celFilter, err := newCELFilter("test", test.code, []string{"false", ""}, false)
			//	require.NoError(b, err)
			trxFilter, err := newCELTrxFilter(test.code, "")
			require.NoError(b, err)

			reusedFilterCache := NewTrxFilterCache()
			preallocatedCaches := make([]*TrxFilterCache, b.N)
			for i := 0; i < b.N; i++ {
				if test.withCache {
					preallocatedCaches[i] = reusedFilterCache
				} else {
					preallocatedCaches[i] = NewTrxFilterCache()
				}
			}
			...
			b.ReportAllocs()
			for i := 0; i < b.N; i++ {
				match, _ := trxFilter.Matches(test.trace, preallocatedCaches[i])
				if !match {
					b.Fail()
				}
			}
