...
main() {
  ...
  archive_file_name=sei-devnet-chain.tar

  primary_endpoint="https://rpc-arctic-1.sei-apis.com"
  secondary_endpoint="https://rpc.arctic-1.seinetwork.io"
  genesis_url="https://github.com/sei-protocol/testnet/raw/main/arctic-1/genesis.json"
  ...
  $sei init sf-firehose-devnet-node --chain-id arctic-1 -o 2> /dev/null
  ...
  cd "$home"
  tar cf "$archive_file_name" *
  zstd -q --no-progress "$archive_file_name"

  mv "${archive_file_name}.zst" "$ROOT/${archive_file_name}.zst"
