...
func (c *Cache) initialize(ctx context.Context, startBlock, endBlock uint64) *cache {
	...
	err := c.store.Walk(ctx, cacheFileStartBlockPrefix(prefixSearchBlock), ".tmp", func(fn string) (err error) {
		filename = fn
		found = true
		return nil
	})
	...
	if !found {
		...
		err := c.store.Walk(ctx, cacheFilePrefix(), ".tmp", func(fn string) (err error) {
