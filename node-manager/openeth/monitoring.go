...
func (s *Superviser) Monitor() {
	...
	for {
		...
		resp, err := s.sendIPCCommand(`{"jsonrpc":"2.0","method":"parity_enode","params":[],"id":1}`)
		if err != nil {
			...
			s.setEnodeStr(gjson.Get(resp, "result").String())
		}

		resp, err = s.sendIPCCommand(`{"method":"parity_netPeers","params":[],"id":1,"jsonrpc":"2.0"}`)
		if err != nil {
			s.Logger.Warn("geth Monitor cannot get peers from IPC socket", zap.Error(err))
		} else {
			connectedPeers := []string{}
			for _, peer := range gjson.Get(resp, "result.peers").Array() {
				connectedPeers = append(connectedPeers, peer.Get("id").String())
			}
			s.infoMutex.Lock()
		...
		if s.headBlockUpdateFunc != nil {
			resp, err = s.sendIPCCommand(`{"jsonrpc":"2.0","method":"eth_blockNumber","params":[],"id":1}`)
			if err != nil {
			...
			resp, err = s.sendIPCCommand(fmt.Sprintf(`{"jsonrpc":"2.0","method":"eth_getBlockByNumber","params":["%s", true],"id":1}`, lastBlock))
			if err != nil {
